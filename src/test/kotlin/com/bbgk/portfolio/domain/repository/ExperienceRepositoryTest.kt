...
class ExperienceRepositoryTest(
    ...
    fun beforeAll() {
        ...
        println("---테스트 데이터 초기화 조회 시작---")
        val experiencs = mutableListOf<Experience>()
        for (i in 1..DATA_SIZE) {
            val experience = createExperience(i)
            experiencs.add(experience)
        }
        experienceRepository.saveAll(experiencs)
        println("---테스트 데이터 초기화 조회 종료---")
    }
